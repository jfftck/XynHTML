...
export async function example17(output) {
    ...
    const renderRoute = router.routes(
        ...
        route(pathMatcher("settings"), exactRouting(currentRoute), "settings"),
    );
        ...
        switch (currentRoute.value) {
            case "dashboard":
                ...
                paragraph.children.add(
                    text(
                        "Welcome to your dashboard! This route uses exact matching.",
                    ),
                );
            ...
            case "user":
                heading.children.add(
                    text(`👤 User Profile #${userIdParam.value}`),
                );
                paragraph.children.add(
                    text(
                        `Viewing details for user ID: ${userIdParam.value}. This demonstrates path parameters!`,
                    ),
                );
            ...
            default:
                ...
                paragraph.children.add(
                    text(
                        "Click a navigation link above to see routing in action.",
                    ),
                );
        }
    ...
    infoText.children.add(
        text(
            "💡 Try navigating to /user/999 by clicking User links above. The path parameter is extracted automatically!",
        ),
    );
